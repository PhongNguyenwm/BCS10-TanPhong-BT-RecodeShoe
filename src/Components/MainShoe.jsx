...
export default class MainShoe extends Component {
  state = {
    shoeArr: [
      {
        id: 1,
  ...
  handleAddToCart = (shoe) => {
    ...
    //2 dòng trên sử dụng trước khi set điều kiện giỏ hàng
  ...
  handleDeleteItem = (itemId) => {
    let deleteItem = this.state.cart.filter((item) => item.id !== itemId);
    this.setState({ cart: deleteItem });
  };

  handleAmountChange = (itemId, increase) => {
    let updateCart = this.state.cart.map((item) => {
      if (item.id === itemId) {
        if (increase) {
